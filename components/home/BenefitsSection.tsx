...
export function BenefitsSection() {
  ...
  const cutoutY = useTransform(scrollYProgress, [0, 1], [25, -25]);
  ...
  return (
    ...
    >
      <div
        style={{ display: "grid", gridTemplateColumns: "1.3fr 0.7fr", alignItems: "stretch" }}
        className="grid-cols-1 md:grid-cols-[1.3fr_0.7fr]"
      >
        {/* Left — text */}
        <div style={{ padding: "clamp(4rem, 9vw, 5.75rem) clamp(2.5rem, 6vw, 6.25rem)", order: 1 }}>
          <h2 style={{
            ...
            Real care.<br />
            {/* Indent second line slightly for editorial rhythm */}
            <em style={{ fontStyle: "italic", paddingLeft: "clamp(1.5rem, 3vw, 3rem)", display: "block" }}>Real change.</em>
          </h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4" style={{ gap: "clamp(1.5rem, 3vw, 2rem)" }}>
            {benefits.map((b, i) => (
              <div key={b.title}>
                ...
                <p style={{ fontFamily: "var(--font-body)", fontSize: "1rem", fontWeight: 600, color: "#292b25", margin: "0 0 0.375rem", lineHeight: 1.3 }}>
                  {b.title}
                </p>
                <p style={{ fontFamily: "var(--font-body)", fontSize: "0.875rem", lineHeight: 1.65, color: "#64685f", margin: 0 }}>
                  {b.description}
                </p>
              </div>
            ))}
        ...
        {/* Right — image with floating product cutout */}
        <div style={{ position: "relative", minHeight: "clamp(360px, 50vw, 640px)", backgroundColor: "#ddd8cd", order: 2, overflow: "visible" }}>
          <Image
          ...
          {/* Product bottle crossing into text panel */}
          <motion.div
            style={{
              position: "absolute",
              left: -90, bottom: 40,
              width: 160, zIndex: 3,
              filter: "drop-shadow(0 24px 30px rgba(35,40,30,0.14))",
              y: cutoutY,
            }}
          >
            <Image
              src={homepageImages.productCutout}
              alt=""
              width={160}
              height={400}
